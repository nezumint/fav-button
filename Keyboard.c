...
void UserInit(void)
{
    ...
    //Initialize buttons
    ANSELHbits.ANS10 = 0;   //RB4 set digital mode
    TRISBbits.TRISB4 = 1;   //main button input
    TRISCbits.TRISC6 = 1;   //dummy
    INTCON2bits.RABPU = 0;  //enable pull-up

    //LED PWM setting
    TRISCbits.TRISC5 = 0;   //LED pin output
    CCP1CON |= 0x0c;
    T2CON |= 0x06;
    PR2 = 0xff;
    CCPR1L = 0x00;
...
void Keyboard(void)
{
    static unsigned char ledcnt;
    static unsigned int cntskip;
    unsigned char i;
    ...
    if(!HIDTxHandleBusy(lastINTransmission))
    {
        ...
        if(!PORTBbits.RB4){
            hid_report_in[0] = 0x01;    //Ctrl
            hid_report_in[2] = 0x16;    //S
            ledcnt = 0xff;

//        }else if(!(PORTB & 0x02)){
//            hid_report_in[2] = 0x52;    //up
//
//        }else if(!(PORTB & 0x04)){
//            hid_report_in[2] = 0x51;    //down
        }
